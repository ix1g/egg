...
// ARABIC VERSION JUST CHANGE THIS FILE to index.js if you want it Arabic!
...
async function getMinecraftServerStatus() {
    try {
        ...
        let playerList = players.list ? players.list.join(', ') : 'لا يوجد لاعبين!';
        ...
        const statusEmbed = new EmbedBuilder()
            ...
            .addFields(
                { name: 'Server Online:', value: online ? 'شغال' : 'طافي', inline: true },
...
client.on('messageCreate', async (message) => {
    if (message.content === '!start') {
        ...
        if (!process.env.CHANNEL_ID) {
            return message.reply('الروم المحددة ليست موجود!');
        }

        message.reply('يتم تشغيل الحالة بنجاح!');
